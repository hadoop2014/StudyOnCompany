...
select
    ...
    a.应付职工薪酬（期末余额）,
    a.应付职工薪酬（期初余额）,
    round((replace(a.支付给职工及为职工支付的现金,',','') + a.应付职工薪酬（期末余额） - a.应付职工薪酬（期初余额）)
        /replace(a.在职员工的数量合计,',','')/10000,2)
        as [员工人均工资（万）],
    round((replace(a.支付给职工及为职工支付的现金,',','') + a.应付职工薪酬（期末余额） - a.应付职工薪酬（期初余额）)/replace(a.营业收入,',',''),4)
        as 员工工资占营业收入比率,
    a.营业收入,
    a.归属于上市公司股东的净利润,
    ...
    case when a.归属于上市公司股东的扣除非经常性损益的净利润（上期） != '' and a.归属于上市公司股东的扣除非经常性损益的净利润（上期） is not NULL
        then round((a.归属于上市公司股东的扣除非经常性损益的净利润 - a.归属于上市公司股东的扣除非经常性损益的净利润（上期）)/a.归属于上市公司股东的扣除非经常性损益的净利润（上期）,4)
        else 0 end
    ...
    a.研发费用,
    a.资本化研发投入,
    a.销售费用 + a.管理费用 + a.财务费用 as 三费总额（销售管理财务）,
    a.资产减值准备,
    ...
    round((a.流动资产合计 - replace(a.存货,',',''))/a.流动负债合计,2) as 速动比率,
    case when (replace(a.短期借款,',','') + replace(a.一年内到期的非流动负债,',','')) = 0 then 100.0
        else round(replace(a.六、期末现金及现金等价物余额,',','')/(replace(a.短期借款,',','') + replace(a.一年内到期的非流动负债,',','')),2)
        end as 现金及现金等价物余额占短期借债的比率,
    round(a.流动资产合计/replace(a.负债合计,',',''),2) as 流动资产占总负债的比率,
    a.销售商品、提供劳务收到的现金,
    ...
    case when a.存货 != 0 then round(replace(a.营业成本,',','')/replace(a.存货,',',''),2) else 100000.0 end
        as 存货周转率,
    round(replace(a.三、营业利润,',','')/(a.流动资产合计 - a.流动负债合计),2) as 营业利润占营业资金的比率,

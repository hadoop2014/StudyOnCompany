...
(
    ...
    from
    (   -- 同仁堂2019年报中 出现两行 2019年 的数据, 其中现金分红金额 少的那一行,实际是半年度分红, 需要将其去掉
        select 报告时间, 公司代码, 报告类型, 货币单位,
             max(现金分红金额（含税）) as 现金分红金额（含税）,
             max(现金分红金额占合并报表中归属于上市公司普通股股东的净利润的比率) as 现金分红金额占合并报表中归属于上市公司普通股股东的净利润的比率
        from 年度普通股现金分红情况表
        group by 报告时间, 公司代码, 报告类型, 货币单位
    ) x
)d
left join {0}合并利润表 e
left join (
...
(
    ...
    left join
    (
        --骆驼股份2017年报,无形资产情况, 出现两个土地使用权,取大的那一条记录
        select 报告时间,公司代码,报告类型,公司简称,max(期末账面价值) as 期末账面价值
        from
        (
